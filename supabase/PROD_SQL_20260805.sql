-- ════════════════════════════════════════════════════════════════════════
-- ПРОД-МИГРАЦИЯ 2026-08-05 — напоминания о данных водителя + route_match
-- Применять на проде одним куском (идемпотентно). Соответствует файлу:
--   supabase/migrations/20260805000001_driver_info_reminders_and_route_match.sql
--
-- Все изменения аддитивные: старый фронт от них не ломается.
--
-- ВАЖНО — перед тем как cron начнёт слать ПИСЬМА (уведомления в колокольчике
-- заработают сразу и без этого шага): выполнить один раз в SQL-редакторе
-- Supabase со своим случайным значением и продублировать то же значение в
-- переменной окружения CRON_SECRET на Vercel:
